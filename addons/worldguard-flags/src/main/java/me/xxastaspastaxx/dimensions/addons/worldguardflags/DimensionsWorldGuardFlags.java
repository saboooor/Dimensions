...
public class DimensionsWorldGuardFlags extends DimensionsAddon implements Listener {
  ...
  public void onPortalIgnite(CustomPortalIgniteEvent e) {
    if (testWorldGuard(
        e.getEntity(), e.getCompletePortal().getCenter(), WorldGuardFlags.IgniteCustomPortal)) {
      e.setCancelled(true);
    }
  }
  ...
  public void onPortalDestroy(CustomPortalBreakEvent e) {
    if (testWorldGuard(
        e.getDestroyer(), e.getCompletePortal().getCenter(), WorldGuardFlags.DestroyCustomPortal)) {
      e.setCancelled(true);
    }
  }
  ...
  public void onPortalUse(CustomPortalUseEvent e) {
    if (testWorldGuard(
        e.getEntity(), e.getCompletePortal().getCenter(), WorldGuardFlags.UseCustomPortal)) {
      e.setCancelled(true);
    }
  }
