...
public class DimensionsBiomes extends DimensionsAddon implements Listener {
  ...
  public void onPortalIgnite(CustomPortalIgniteEvent e) {
    ...
    Object allowedBiomes = getOption(complete, "biomesEnabled");
    if (allowedBiomes == null) return;

    Entity entity = e.getEntity();

    ArrayList<Biome> enabled = (ArrayList<Biome>) getOption(complete, "biomesEnabled");
    ArrayList<Biome> disabled = (ArrayList<Biome>) getOption(complete, "biomesDisabled");

    Biome biome = complete.getCenter().getBlock().getBiome();
    if (disabled.contains(biome) || (!enabled.isEmpty() && !enabled.contains(biome))) {
      if (entity instanceof Player)
        entity.sendMessage((String) getOption(complete, "biomesDenyMessage"));
      e.setCancelled(true);
    }
  }
  ...
  public void registerPortal(YamlConfiguration portalConfig, CustomPortal portal) {
    ...
    try {
      ...
      if (allowedBiomes.contains("all") && allowedBiomes.contains("!")) {
        ...
        if (!allowedBiomes.contains(", ")) {
          if (!allowedBiomes.startsWith("!"))
            enabled.add(Biome.valueOf(allowedBiomes.toUpperCase()));
        } else {
      ...
      DimensionsDebbuger.HIGH.print("BiomesAddon error: " + e.getMessage());
    }
    ...
    setOption(
        ...
        "biomesDenyMessage",
        portalConfig
            .getString("Addon.Biomes.DenyMessage", "The portal cannot be activate in this biomes")
            .replace("&", "§"));
