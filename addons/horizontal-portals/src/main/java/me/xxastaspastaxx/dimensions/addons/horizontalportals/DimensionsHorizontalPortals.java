...
import me.xxastaspastaxx.dimensions.completePortal.PortalEntityText;
...
public class DimensionsHorizontalPortals extends DimensionsAddon implements Listener {
      ...
      for (double z = min.getZ(); z <= max.getZ(); z++) {
        ...
        if (customPortal.getInsideMaterial() != null) {
          ...
          if (customPortal.getInsideMaterial().isSolid()
              ...
              || customPortal.getInsideMaterial() == Material.END_GATEWAY) {
            entity =
                new PortalEntitySand(
                    loc, customPortal.getCombinedID(false), customPortal.getInsideSprite() != null);
          } else {
            entity =
  ...
  public void postPortalUse(CustomPortalUseEvent e) {
    ...
    PortalGeometry geom =
        PortalGeometry.getPortalGeometry(customPortal)
            .getPortal(customPortal, newLocation.add(1.5, 0, 1.5));
    if (geom == null) return;
    destination =
        Dimensions.getCompletePortalManager()
            .createNew(
                new CompletePortal(customPortal, destinationWorld, geom),
                null,
  ...
  private Location getSafeLocation(
      CustomPortal customPortal, Location newLocation, World destinationWorld, int width) {
    Location backupLocation = null;
    Location checkLocation;

    //		int minWorldHeight = (int)
    // DimensionsSettings.get("Worlds."+destinationWorld.getName()+".MinHeight", 0);
    //		int maxWorldHeight = (int)
    // DimensionsSettings.get("Worlds."+destinationWorld.getName()+".MaxHeight",
    // destinationWorld.getMaxHeight());
  ...
  private boolean canBuildPortal(
    ...
    for (double x = checkLocation.getX(); x <= maxX; x++) {
      for (double z = checkLocation.getZ(); z <= maxZ; z++) {
        ...
        if (checkPlatform && !block.getRelative(BlockFace.DOWN).getType().isSolid())
          return false; // check if has bottom

        if ((x == checkLocation.getX() || x == maxX)
            || ((z == checkLocation.getZ()) || z == maxZ)) {
