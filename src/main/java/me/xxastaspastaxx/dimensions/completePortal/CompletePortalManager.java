...
public class CompletePortalManager {
   ...
   */
  public List<CompletePortal> getCompletePortals(
      Location location, boolean outside, boolean corner) {
   ...
   */
  public ArrayList<CompletePortal> getNearestPortals(
      Location loc, int searchRadius, CustomPortal customPortal) {
   ...
   */
  public CompletePortal getNearestPortal(
      Location teleportLocation,
      CompletePortal sample,
      double ratio,
      boolean sameAxis,
      boolean sameSize,
      Predicate<CompletePortal> predicate) {
    double closestDistance = Math.pow(DimensionsSettings.searchRadius * ratio, 2);
    CompletePortal closestPortal = null;
    for (CompletePortal complete : completePortals) {
      if (!complete.getCustomPortal().equals(sample.getCustomPortal())
          || !complete.getCenter().getWorld().equals(teleportLocation.getWorld())) continue;
      if (DimensionsSettings.ignoreLinkedPortals && complete.getLinkedPortal() != null) continue;
      if (sameAxis
          && sample.getPortalGeometry().iszAxis() != complete.getPortalGeometry().iszAxis())
        continue;
      if (sameSize
          && (sample.getPortalGeometry().getPortalWidth()
                  != complete.getPortalGeometry().getPortalWidth()
              || sample.getPortalGeometry().getPortalHeight()
                  != complete.getPortalGeometry().getPortalHeight())) continue;
      if (!predicate.test(closestPortal)) continue;
      Location temp = complete.getCenter();
      double dist =
          NumberConversions.square(temp.getX() - teleportLocation.getX())
              + NumberConversions.square(temp.getZ() - teleportLocation.getZ());
   ...
   */
  public CompletePortal createNew(
      CompletePortal completePortal,
      Entity igniter,
      CustomPortalIgniteCause cause,
      ItemStack item) {

    if (completePortal.getPortalGeometry() == null) return null;

    CustomPortalIgniteEvent igniteEvent =
        new CustomPortalIgniteEvent(completePortal, cause, igniter, item);
  ...
  public boolean removePortal(
      CompletePortal completePortal, CustomPortalDestroyCause cause, Entity destroyer) {

    DimensionsDebbuger.DEBUG.print("DESTROYED BY " + cause + " " + destroyer);
    CustomPortalBreakEvent breakEvent =
        new CustomPortalBreakEvent(completePortal, cause, destroyer);
