...
public class Dimensions extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    DimensionsDebbuger.MEDIUM.print(
        "Found " + customPortalManager.getCustomPortals().size() + " portals.");
    ...
    // Portals require a world instance in order to be loaded and we can only have that if the
    // plugin has loaded the required worlds
    ...
    metrics.addCustomChart(
        new Metrics.DrilldownPie(
            "portal_blocks_frames",
            () -> {
              Map<String, Map<String, Integer>> map = new HashMap<>();
              for (CustomPortal portal : getCustomPortalManager().getCustomPortals()) {
                if (portal.getInsideMaterial() != null) {
