...
public abstract class DimensionsAddon {
  ...
  private static HashMap<CompletePortal, HashMap<String, Object>> addonOptionsOverride =
      new HashMap<CompletePortal, HashMap<String, Object>>();
  private static HashMap<CustomPortal, HashMap<String, Object>> addonOptions =
      new HashMap<CustomPortal, HashMap<String, Object>>();
  ...
  public static void setOption(CustomPortal portal, String key, Object value) {
    ...
    if (value == null) addonOptions.get(portal).remove(key);
    else addonOptions.get(portal).put(key, value);
  }
  ...
  public static void setOption(CompletePortal complete, String key, Object value) {
    if (!addonOptionsOverride.containsKey(complete))
      addonOptionsOverride.put(complete, new HashMap<String, Object>());
    if (value == null) addonOptionsOverride.get(complete).remove(key);
    else addonOptionsOverride.get(complete).put(key, value);
  }

  /**
   * Get the option for the portal. If the portal does not contain the key, then we check the type
   * of the portal for the given key
   *
  ...
  public static Object getOption(CompletePortal complete, String key) {
    if (addonOptionsOverride.containsKey(complete)
        && addonOptionsOverride.get(complete).containsKey(key)) {
  ...
  /**
   * This function provides the portal config file for each portal that is being loaded and addons
  ...
  /**
   * This runs when we reload the plugin <b>AFTER</b> all the addons have been disabled in order to
   * reset all the data
   */
