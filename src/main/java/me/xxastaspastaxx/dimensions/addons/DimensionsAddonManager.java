...
public class DimensionsAddonManager {
  ...
  /**
   * Constructor of the Addon manager. Creates the directory containing the addons and loads all the
   * addons using ServiceLoader
   *
  ...
  public DimensionsAddonManager(Dimensions pl) {
    ...
    loader =
        ServiceLoader.load(
            DimensionsAddon.class,
            URLClassLoader.newInstance(this.urls, Dimensions.class.getClassLoader()));
    ...
    while (iter.hasNext()) {
      try {
        ...
        if (addon.onLoad(pl)) {
          ...
          DimensionsDebbuger.MEDIUM.print(
              "Failed to load addon: " + addon.getName() + " v" + addon.getVersion());
        }
  ...
  public void enableAddons() {
    for (DimensionsAddon addon : loadedAddons) {
      DimensionsDebbuger.MEDIUM.print(
          "Enabling addon: " + addon.getName() + " v" + addon.getVersion());
      addon.onEnable(pl);
    }
  }
  ...
  ArrayList<String> dontUnload =
      new ArrayList<String>(
          Arrays.asList(
              new String[] {
                "me.xxastaspastaxx.dimensions.listener.PortalListener",
  ...
  public boolean unload(DimensionsAddon plugin) {
    ...
    HandlerList.getRegisteredListeners(pl).stream()
        .filter(r -> !dontUnload.contains(r.getListener().getClass().getName()))
        .forEach(r -> HandlerList.unregisterAll(r.getListener()));
    //
